...
const Character: React.FC<Props> = ({ image, left, top, active, user, raise, turn, index, onFold, items, ice, xp, dg }) => {
    ...
    return (
        <Box left={left} top={top} position="absolute" >
            ...
            {!user && <InfoDialog open={infomodalopen} setOpen={setInfoModalOpen} items={items} index={index} />}
            {user && <UserInfoDialog open={infomodalopen} setOpen={setInfoModalOpen} items={items} ice={ice} xp={xp} dg={dg} />}
            <PlayerCircle active={active} onClick={() => setInfoModalOpen(!infomodalopen)}>
